  ...
  <div class="animated fadeInLeft" v-if="scheduleItem != null">
            ...
            <div class="flex justify-around px-1" style="width: 70%">
              <div class="relative">
                <div class="flex justify-center items-center">
                  <img src="@/assets/images/fastthree/icon-circle-active-red.png" class="w-[48px]" />
    ...
    <van-dialog v-model:show="descriptionShow" :title="descriptionTitle">
      <div class="p-2 h-[400px] overflow-y-scroll">
        <p>该游戏的投注时间、开奖时间和开奖号码与广西快乐十分完全同步，北京时间（GMT+8）每天白天从上午09：00开到晚上22：20，每 20 分钟开一次奖，每天开奖 40 期。</p>
...
const descriptionTitle = ref("广西快乐十分游戏规则");
const time = ref(0);
...
const lotteryStatus = computed(() => {
  ...
  if (Number(getLotteryStatus.value.gxsf.close) === 1) {
    is_open.value = false;
    closed_reason.value = getLotteryStatus.value.gxsf.des;
...
const showPopUp = () => {
  ...
  if (selectedItemList.value.length == 0) {
    ...
    if (g_type.value == "gxsf" && Number(lotteryUserConfigItem.value.gxsf_max_bet) == 0) {

  ...
  <div class="animated fadeInLeft" v-if="scheduleItem != null">
    ...
    <van-dialog v-model:show="descriptionShow" :title="descriptionTitle">
      <div class="p-2 h-[400px] overflow-y-scroll">
        <p>每期广东快乐十分开奖球数共八粒。每粒球除了总和玩法，其它都有单独的投注页面。广东快乐十分每天开42期，每期间隔20分钟。投注时间为18分钟，等待开奖时间为2分钟，北京时间（GMT+8）每天白天从上午09：00开到晚上23：00。</p>
...
const title = ref("广东十分彩");
const g_type = ref("gdsf");
...
const descriptionTitle = ref("广东快乐十分游戏规则");
...
const showPopUp = () => {
  ...
  if (selectedItemList.value.length == 0) {
    ...
    if (g_type.value == "gdsf" && selectedBetAmount.value > lotteryUserConfigItem.value.gdsf_max_bet) {
      showToast("该彩票单注最高金额：" + lotteryUserConfigItem.value.gdsf_max_bet)
      return;
    }
    if (g_type.value == "gdsf" && selectedBetAmount.value < lotteryUserConfigItem.value.gdsf_lower_bet) {
      showToast("该彩票单注最低金额：" + lotteryUserConfigItem.value.gdsf_lower_bet)
      return;
    }

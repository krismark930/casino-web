  ...
  <div class="animated fadeInLeft" v-if="scheduleItem != null">
    ...
    <van-dialog v-model:show="descriptionShow" :title="descriptionTitle">
      <div class="p-2 h-[400px] overflow-y-scroll">
        <p>每期重庆快乐十分开奖球数共八粒。每粒球除了总和玩法，其它都有单独的投注页面。重庆快乐十分每天开49期，每期间隔20分钟。投注时间为18分钟，等待开奖时间为2分钟，北京时间（GMT+8）每天白天从上午07：00开到晚上20:00，凌晨 00:00-02：50。</p>
    ...
    <van-dialog v-model:show="alertShow" title="提示">
      ...
      <div class="text-center">重庆快乐十分开盘时间为：07:00 - 20:10 凌晨00:00-02:50</div>
    </van-dialog>
...
import OneWordPosition from "./cqsf/OneWordPosition.vue";
import DragonTigerSum from "./cqsf/DragonTigerSum.vue";
...
const title = ref("重庆十分彩");
const g_type = ref("cqsf");
const closed_reason = ref("目前没有开盘!");
const descriptionTitle = ref("重庆快乐十分游戏规则");
const time = ref(0);
...
const lotteryStatus = computed(() => {
  ...
  if (Number(getLotteryStatus.value.cqsf.close) === 1) {
    is_open.value = false;
    closed_reason.value = getLotteryStatus.value.cqsf.des;
...
const showPopUp = () => {
  ...
  if (selectedItemList.value.length == 0) {
    ...
    if (g_type.value == "cqsf" && Number(lotteryUserConfigItem.value.cqsf_max_bet) == 0) {
      showToast("该彩票注单最高金额：0。00");
      return;
    }

    if (g_type.value == "cqsf" && selectedBetAmount.value > lotteryUserConfigItem.value.cqsf_max_bet) {
      showToast("该彩票单注最高金额：" + lotteryUserConfigItem.value.cqsf_max_bet)
      return;
    }
    if (g_type.value == "cqsf" && selectedBetAmount.value < lotteryUserConfigItem.value.cqsf_lower_bet) {
      showToast("该彩票单注最低金额：" + lotteryUserConfigItem.value.cqsf_lower_bet)
      return;
    }

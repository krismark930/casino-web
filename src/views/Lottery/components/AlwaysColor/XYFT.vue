  ...
  <div class="animated fadeInLeft" v-if="scheduleItem != null">
    ...
    <van-dialog v-model:show="descriptionShow" :title="descriptionTitle">
      <div class="p-2 h-[400px] overflow-y-scroll">
        <p>该游戏的开奖时间为该游戏的开奖时间为北京时间（GMT+8）每天白天从上午13:04开到晚上04:04，每5分鐘开一次奖,每天开奖180期。</p>
    ...
    <van-dialog v-model:show="alertShow" title="提示">
      ...
      <div class="text-center">幸运飞艇开盘时间为：13:00 - 04:05</div>
    </van-dialog>
...
const title = ref("幸运飞艇");
const g_type = ref("xyft");
...
const descriptionTitle = ref("幸运飞艇游戏规则");
...
const showPopUp = () => {
  ...
  if (selectedItemList.value.length == 0) {
    ...
    if (g_type.value == "xyft" && Number(lotteryUserConfigItem.value.xyft_max_bet) == 0) {
      showToast("该彩票注单最高金额：0。00");
      return;
    }

    if (g_type.value == "xyft" && selectedBetAmount.value > lotteryUserConfigItem.value.xyft_max_bet) {
      showToast("该彩票单注最高金额：" + lotteryUserConfigItem.value.xyft_max_bet)
      return;
    }
    if (g_type.value == "xyft" && selectedBetAmount.value < lotteryUserConfigItem.value.xyft_lower_bet) {
      showToast("该彩票单注最低金额：" + lotteryUserConfigItem.value.xyft_lower_bet)
      return;
    }

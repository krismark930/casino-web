    ...
    <div class="w-full border-[#2674ff] border rounded-sm bg-white pt-[4px] pb-[6px]">
        <div class="flex justify-center items-center">
            <div class="flex justify-center">
                <div class="relative">
                    <img :src="item.icon" class="w-[45px]" alt="dice" v-if="item.icon !== ''" />
                    <div class="text-black font-bold mt-[2px] mr-[4px] text-[16px]" v-else>{{ item.name }}</div>
                    <div class="absolute text-black number-position font-bold text-[18px]" v-if="item.num != ''">
                        {{ item.num }}
                    </div>
                </div>
            </div>

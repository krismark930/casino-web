  ...
  <div class="animated fadeInLeft" v-if="scheduleItem != null">
    ...
    <div class="pt-[47px] pb-[60px] h-screen bg-gray-100 text-[15px]">
      ...
      <div v-if="is_open" class="overflow-y-auto" style="height: calc(100vh - 200px)">
        ...
        <Company3 :lotteryOddsList="lotteryOddsList" :initialize="initialize" :disabled="disabled"
          @submitItem3="submitItem3" />
        <Span :lotteryOddsList="lotteryOddsList" :initialize="initialize" :disabled="disabled"
    ...
    <van-dialog v-model:show="descriptionShow" :title="descriptionTitle">
      <div class="p-2 h-[400px] overflow-y-scroll">
        ...
        <p>根据相应单项投注的第一球 ~ 第三球开出的球号大于或等于 5 为特码大，小于或等于 4 为特码小。</p>
        <div class="text-blue-500">单双：</div>
        <p>根据相应单项投注的第一球 ~ 第三球开出的球号为双数则为特码双，如 2、6；球号为单数则为特码单，如 1、3。</p>
        ...
        <div class="font-bold text-red-500">3、三连特殊玩法： 豹子 > 顺子 > 对子 > 半顺 > 杂六</div>
        ...
        <div class="font-bold text-red-500">4、龙虎和特殊玩法： 龙 > 虎 > 和 （0为最小，9为最大）</div>
        <div class="text-blue-500">龙：</div>
        <p>开奖第一球（百位）的号码 大于 第三球（个位）的号码。如：第一球开出 6，第三球开出 2、第一球开出 8，第三球开出 6、第一球开出 5，第三球开出 1...开奖为龙，投注龙者视为中奖，其它视为不中奖。</p>
        <div class="text-blue-500">虎：</div>
        <p>开奖第一球（百位）的号码 小于 第三球（个位）的号码。如：第一球开出 2，第三球开出 6、第一球开出 6，第三球开出 8、第一球开出 1，第三球开出 5...开奖为虎，投注虎者视为中奖，其它视为不中奖。</p>
        ...
        <div class="font-bold text-red-500">5、跨度</div>
        <div class="text-blue-500">跨度：</div>
...
const onFinish = async () => {
  if (scheduleItem.value.is_open) {
    ...
    await dispatchBirthHistory({ g_type: g_type.value, type: "b3" });
...
const scheduleItem = computed(() => {
  ...
  // let end = g_type.value == "t3" ? moment(current_date + " " + getScheduleItem.value.kaijiang_time) : moment(getScheduleItem.value.kaijiang_time);

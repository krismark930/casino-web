...
export default defineComponent({
	...
	sockets: {
		...
		receivedBKLeagueChampionMessage(data: any) {
			this.loading = false;
			if (data == null) return;
	...
	created() {
		this.loading = true;
		this.$socket.emit("sendBKLeagueChampionMessage");
	},
	unmounted() {
		this.$socket.emit("stopBKLeagueChampionMessage");
	}
})
</script>

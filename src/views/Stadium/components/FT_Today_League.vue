				...
				<div v-if="regionItem['show']">
					<div v-if="Array.isArray(regionItem.league)">
						<van-cell :title=leagueItem._attributes.name v-for="(leagueItem, leagueIndex) in regionItem.league"
							:key="leagueIndex" @click="showLIDDetail(leagueItem._attributes.lid)" />
					</div>
					<div v-else>
						<van-cell :title=regionItem.league._attributes.name
							@click="showLIDDetail(regionItem.league._attributes.name)" />
					</div>
				</div>
			</div>
		</div>
...
export default defineComponent({
	...
	methods: {
		showCouponDetail: function (lids, name) {
			switch (name) {
				case "今日赛事":
					this.$emit("showTodayMain", lids, "cpl");
					break;
				case "欧冠":
					this.$emit("showTodayMain", lids, "HotGame_FT_lid_0");
					break;
				case "欧洲顶级联赛":
					this.$emit("showTodayMain", lids, "HotGame_FT_lid_1");
					break;
				case "南美顶级赛事":

			...
			<div class="hall_box animated fadeInLeft">
				<div class="tab_bottom">
					<div class="tab_bottom_box">
						<div class="tab_bottom_item" v-for="(item, index) in chessGameList" :key='index' @click="goDetail(item.GameType)">
...
import { chessGameStore } from '@/stores/chess_game';
...
const swipeItemList = ref([
	{
		id: 1,
		icon: FILE_BASE_URL + "storage/upload/zr_images/chess/180x180/dzpk.png"
	},
	{
		id: 2,
		icon: FILE_BASE_URL + "storage/upload/zr_images/chess/180x180/ddz.png"
	},
	{
		id: 3,
		icon: FILE_BASE_URL + "storage/upload/zr_images/chess/180x180/sg.png"
	},
])
const onClickLeft = () => {
...
const goDetail = async (KindID: Number) => {
	...
	await dispatchRedirectUrl({KindID}, token.value);
	loading.close();
    if (success.value) {
        if (redirectKyUrl.value != "") {
            window.open(redirectKyUrl.value, '_blank');
        }
    } else {
